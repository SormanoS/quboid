...
use quboid_core::{Action, AppConfig, Language, NormalizedRect, RuntimeEvent};
use quboid_ui::{
    ExtensionChange, ExtensionPage, LocalizedText, PageIcon, Palette, UiExtension, UiIntent,
    UiState,
    theme::{contrast_ratio, style},
};
...
fn sidebar_brand_has_a_clear_vertical_hierarchy() {
    ...
    let title = harness.get_by_label("Quboid").rect();
...
fn shortcut_editor_uses_responsive_columns_and_keeps_the_status_at_the_bottom() {
    ...
    let status_y = harness.get_by_label("Quboid è pronto").rect().center().y;
...
fn generated_values_avoid_the_ones_already_taken() {
    assert_eq!(
        quboid_ui::unique_id("zone", ["zone-1", "zone-3"].into_iter()),
        "zone-2"
    );
    assert_eq!(
        quboid_ui::unique_value(
...
fn sidebar_footer_stays_compact_and_anchored_to_the_bottom() {
    ...
    let status = harness.get_by_label("Quboid è pronto").rect();
...
fn controls_stay_visible_on_the_card_they_are_drawn_on() {
    for (name, dark) in [("dark", true), ("light", false)] {
        ...
        let track = style(dark).visuals.widgets.inactive.bg_fill;
