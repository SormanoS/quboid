...
impl UiState {
    pub fn new(config: AppConfig) -> Self {
        let status = text(config.language, "Quboid è pronto", "Quboid is ready").to_owned();
        Self {
            config,
            page: Page::default(),
            status,
    ...
    fn page_header(&self, ui: &mut Ui) {
        ...
        let (title, description) = match self.page {
            ...
            Page::General => (
                ...
                LocalizedText::new(
                    "Configura il comportamento e le preferenze di Quboid.",
                    "Configure Quboid behavior and preferences.",
                ),
            ),
        };
        let palette = Palette::of(ui);
                ...
                .show(ui, |ui| {
                    ...
                    changed |= ui
                        .checkbox(
                            ...
                            text(language, "Avvia Quboid", "Start Quboid"),
                        )
                        .changed();
                    ui.end_row();
                });
