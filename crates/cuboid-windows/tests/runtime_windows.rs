...
use cuboid_windows::{Runtime, RuntimeAdapters, RuntimeError, WindowFilter, set_launch_at_login};
...
fn launch_at_login_writes_and_removes_the_current_user_run_value() {
    ...
    let original = run.get_value::<String, _>("Cuboid").ok();
    let _restore = RegistryValueRestore {
        original: original.clone(),
    };
    let executable = std::env::current_exe().unwrap();

    set_launch_at_login("Cuboid", true, &executable).unwrap();
    assert_eq!(
        run.get_value::<String, _>("Cuboid").unwrap(),
        format!("\"{}\"", executable.display())
    );

    set_launch_at_login("Cuboid", false, &executable).unwrap();
    assert!(run.get_value::<String, _>("Cuboid").is_err());
}
...
impl TestWindow {
    fn new() -> Self {
        ...
        let thread = thread::Builder::new()
            .name("cuboid-test-window".to_owned())
            .spawn(move || {
                let hwnd = unsafe {
                    CreateWindowExW(
                        WINDOW_EX_STYLE::default(),
                        w!("STATIC"),
                        w!("Cuboid runtime integration test"),
...
impl Drop for RegistryValueRestore {
    fn drop(&mut self) {
        ...
        if let Some(original) = &self.original {
            let _ = run.set_value("Cuboid", original);
        } else {
            let _ = run.delete_value("Cuboid");
        }
    }
}
