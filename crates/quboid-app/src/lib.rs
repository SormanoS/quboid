...
impl StorageLocation {
    ...
    /// The log directory for `product_directory`, e.g. `Quboid`.
...
pub fn run(profile: Profile) -> eframe::Result {
    ...
    std::fs::create_dir_all(&log_directory).expect("Quboid could not create its log directory");
    let file_appender = tracing_appender::rolling::daily(log_directory, "quboid.log");
    ...
    let runtime = Runtime::start_with(config.clone(), adapters)
        .expect("Quboid could not start its Windows runtime");
    let commands = runtime.commands();
    let events = runtime.events();
    let tray =
        create_tray_icon(config.language, product).expect("Quboid could not create its tray icon");
...
fn report_appearance_fallbacks(appearance: &WindowsAppearance) {
    ...
    if let RegistrySetting::Unavailable(issue) = &appearance.accent {
        tracing::debug!(
            ?issue,
            "Windows accent is unavailable; Quboid uses its own brand palette anyway"
        );
    }
}
...
mod tests {
    ...
    fn tray_labels_name_the_running_product() {
        assert_eq!(
            tray_labels(Language::Italian, "Quboid"),
            ("Apri Quboid".to_owned(), "Esci".to_owned())
        );
        assert_eq!(
